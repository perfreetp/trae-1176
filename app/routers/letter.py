...
def delete_page(letter_id: int, page_id: int, db: Session = Depends(get_db)):
    page = db.query(LetterPage).filter(
        LetterPage.id == page_id,
        LetterPage.letter_id == letter_id,
    ).first()
...
def set_visibility(letter_id: int, visibility: str, db: Session = Depends(get_db)):
    letter = db.query(Letter).filter(Letter.id == letter_id).first()
    if not letter:
        raise HTTPException(status_code=404, detail="信件不存在")
...
def toggle_star(letter_id: int, db: Session = Depends(get_db)):
    letter = db.query(Letter).filter(Letter.id == letter_id).first()
    if not letter:
        raise HTTPException(status_code=404, detail="信件不存在")
